...
function db_table_exists(PDO $pdo, string $table): bool
{
    ...
    $st->execute([$table]);
    if (function_exists('vk_perf_mark_query')) {
        vk_perf_mark_query();
    }
...
function &db_table_exists_cache(): array
{
    static $cache = [];
    return $cache;
}

/** Clear table-exists cache after DDL (CREATE/DROP). */
function db_table_exists_forget(?string $table = null): void
{
    $cache = &db_table_exists_cache();
    if ($table === null) {
        $cache = [];
